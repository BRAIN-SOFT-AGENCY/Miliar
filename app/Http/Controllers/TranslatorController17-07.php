...
class TranslatorController extends Controller
{
    public function index()
    {
        // echo 'translator';
        // die();
        $books = Book::where('status', 0)->where('type', 1)->where('translatorID', Auth::guard('translator')->user()->translatorID)->whereDate('books.PublierLe', '<=', now())->orderBy('booksID', 'desc')->get();
        $cours = Book::where('status', 0)->where('type', 0)->where('translatorID', Auth::guard('translator')->user()->translatorID)->whereDate('books.PublierLe', '<=', now())->orderBy('booksID', 'desc')->get();
        $etudes = Book::where('status', 0)->where('type', 2)->where('translatorID', Auth::guard('translator')->user()->translatorID)->whereDate('books.PublierLe', '<=', now())->orderBy('booksID', 'desc')->get();
